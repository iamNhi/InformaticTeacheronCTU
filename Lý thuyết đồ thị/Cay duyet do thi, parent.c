...
List depth_first_search(Graph *G, int x, int parent[]) //khai bao them parent
{
    Stack S;
    make_null_stack(&S); //Khoi tao ham rong
    push_stack(&S, x); //dua dinh x vao
    parent[x] = 0; //dinh bat dau, cha = 0
	List list_dfs;
    make_null(&list_dfs);
...
{
    Graph G;
    freopen("Cayduyetdothi.txt", "r", stdin);
    int n, m;
    int i, j;
    scanf("%d%d", &n, &m);
    init_Graph(&G, n);
    ...
    //Khai bao mang luu dinh parent cua mot dinh v trong qua trinh duyet
    int parent[MAX_Vertices]; 
    for(i=1; i<= G.n; i++){
    	parent[i]= -1; //ban dau chua co dinh cha, cho = -1
